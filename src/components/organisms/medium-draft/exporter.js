...
export const getUrlAndAchor = (rawState) => {
    const listStack = [];
    rawState.blocks.map((block) => {
        if (block.entityRanges.length > 0) {
            block.entityRanges.map((entity) => {
                if (block.type !== 'atomic') {
                    return listStack.push({
                        keyword: block.text.substring(entity.offset, entity.offset + entity.length),
                        link: rawState.entityMap[entity.key].data.url,
                    });
                }
            });
        }
        if (block.inlineStyleRanges.length > 0) {
            block.inlineStyleRanges.map((inline) => {
                if (inline.style === 'keyword' || inline.style === 'BOLD') {
                    return listStack.push({
                        keyword: block.text.substring(inline.offset, inline.offset + inline.length),
                    });
                }
            });
        }
...
export const getVideoLink = (rawState) => {
    const listStack = [];
    // console.log(JSON.stringify(rawState));
    rawState.blocks.map((block) => {
        if (block.type === 'atomic') {
            block.entityRanges.map(async (entity) => {
                listStack.push({
                    uri: rawState.entityMap[entity.key].data.url,
                });
            });
        }
        return null;
